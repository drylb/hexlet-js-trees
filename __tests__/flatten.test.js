...
import { flatten, flatten1 } from '../src/flatten';
...
describe('Flatten1', () => {
  it('set 1', () => {
    ...
    expect(flatten1(list)).toEqual([]);
  });

  it('set 2', () => {
    const list = [1, 2, [3, 5], [[4, 3], 2]];

    expect(flatten1(list)).toEqual([1, 2, 3, 5, 4, 3, 2]);
  });

  it('set 3', () => {
    const list = [[1, [5], [], [[-3, 'hi']]], 'string', 10, [[[5]]]];

    expect(flatten1(list)).toEqual([1, 5, -3, 'hi', 'string', 10, 5]);
  });

  it('set 4', () => {
    const list = [1, 2, { a: 1 }, [3, 5], 2];

    expect(flatten1(list)).toEqual([1, 2, { a: 1 }, 3, 5, 2]);
  });
});
