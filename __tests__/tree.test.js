...
import { compressImages, compressImages1 } from '../src/tree';
...
test('compressImages 4', () => {
  ...
  const newTree = compressImages1(tree);
...
test('compressImages 6 - deepClone', () => {
  const tree = mkdir('my documents', [
    mkfile('avatar.jpg', { size: 100, attributes: { hide: false, readOnly: true } }),
    mkdir('presentations'),
  ]);
